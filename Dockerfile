...
COPY ./gppTest.cpp ./wrap_test.cpp ./
RUN g++ -shared -o libcppLib.so -fPIC gppTest.cpp

# RUN g++ -shared -o libCookie.so -fPIC wrap_test.cpp
...
COPY ./secondTest.jl ./binding_cpp.jl ./binding_wrap_cpp.jl ./binding_java.jl ./binding_fort.jl ./binding_all.jl ./

CMD bash -c "julia -e 'include(\"binding_all.jl\"); include(\"binding_cpp.jl\"); include(\"binding_fort.jl\"); include(\"binding_java.jl\");'"